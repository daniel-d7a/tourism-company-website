                    ...
                    <Form {...form} >
                        <form onSubmit={onSubmit} className="grid md:grid-cols-2 gap-5">
                            ...
                            <div className="text-2xl font-medium md:ml-20">
                                            ...
                                            {validOptions?.map((item, index) => (
                                                <div key={index} className="flex flex-row">
                                                    <FormControl>
                                                        <Checkbox
                                                            className="my-3"
                                                            checked={(field.value as TourOption[]).some(obj => obj.name === item.name)}
                                                            onCheckedChange={(checked) => {
                                                                let result = checked
                                                                    ? field.onChange([...field.value, item])
                                                                    : field.onChange((field.value as TourOption[]).filter(
                                                    ...
                                                    <div className="mx-2">
                                                        ...
                                                        <FormDescription className="font-base">extra price: {item.price} $ </FormDescription>
                                                    </div>
                                                </div>
                                            ))}
                                            <FormMessage />
                                ...
                                <FormLabel className="text-xl my-10">total price : {totalPrice} $</FormLabel>
                            </div>
                            <button disabled={!isDirty} className="disabled:bg-gray-600 md:col-span-2 md:w-1/3  block text-2xl text-center font-medium mx-auto  px-5 py-2 rounded-md text-white bg-secondary-foreground hover:bg-secondary" type="submit" >Continue</button>
                        </form>
                    </Form>
                </section >
            }
